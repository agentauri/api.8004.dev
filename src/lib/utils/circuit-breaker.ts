...
export class CircuitBreaker {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (this.state === 'open') {
      ...
      if (timeSinceFailure >= this.config.recoveryTimeout) {
        ...
        // biome-ignore lint/suspicious/noConsole: Circuit breaker state transitions are critical diagnostic logs
  ...
  async executeWithFallback<T>(fn: () => Promise<T>, fallback: () => Promise<T>): Promise<T> {
    try {
      ...
      if (error instanceof CircuitOpenError) {
        // biome-ignore lint/suspicious/noConsole: Circuit breaker fallback is a critical diagnostic log
  ...
  private onSuccess(): void {
    if (this.state === 'half-open') {
      ...
      if (this.halfOpenSuccesses >= this.config.halfOpenRequests) {
        this.state = 'closed';
        this.failures = 0;
        // biome-ignore lint/suspicious/noConsole: Circuit breaker recovery is a critical diagnostic log
  ...
  private onFailure(): void {
    ...
    if (this.state === 'half-open') {
      ...
      this.state = 'open';
      // biome-ignore lint/suspicious/noConsole: Circuit breaker state change is a critical diagnostic log
  ...
  reset(): void {
    ...
    this.lastFailure = 0;
    this.halfOpenSuccesses = 0;
    // biome-ignore lint/suspicious/noConsole: Circuit breaker reset is a critical diagnostic log
